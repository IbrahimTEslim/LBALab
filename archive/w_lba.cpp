...
bool writeToVolume(const std::string& payload, uint64_t sectorOffset, const std::string& volumeLetter) {
    ...
    size_t copySize = min(payload.length(), (size_t)SECTOR_SIZE);
...
int main() {
    ...
    std::string payload = "Hello World22";  // Your data to write
    uint64_t sectorOffset = 0;          // Target sector within the volume (try a low number first!)
